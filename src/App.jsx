...
const baseClass = "rounded-[8px] border border-white/5 bg-white/[0.03]";
const headClass =
...
const foodClass =
  "cell-food rounded-full border border-orange-200/20 bg-gradient-to-br from-orange-300 via-amber-300 to-rose-400 shadow-[0_0_22px_rgba(251,146,60,0.55)]";

const cachedEmptyCells = new Array(BOARD_SIZE * BOARD_SIZE);
for (let i = 0; i < BOARD_SIZE * BOARD_SIZE; i++) {
  cachedEmptyCells[i] = <div key={i} className={baseClass} />;
}
...
function App() {
  ...
  const cells = useMemo(() => {
    // Start with a pre-computed array of empty cell elements
    // to avoid creating 256 React elements on every render
    const result = [...cachedEmptyCells];

    // Override food cell
    const foodIdx = food.y * BOARD_SIZE + food.x;
    result[foodIdx] = <div key={foodIdx} className={foodClass} />;

    // Override snake cells
    if (snake.length > 0) {
      const headIdx = snake[0].y * BOARD_SIZE + snake[0].x;
      result[headIdx] = <div key={headIdx} className={headClass} />;
      for (let i = 1; i < snake.length; i++) {
        const bodyIdx = snake[i].y * BOARD_SIZE + snake[i].x;
        result[bodyIdx] = <div key={bodyIdx} className={bodyClass} />;
      }
    }
        ...
        <section className="relative overflow-hidden rounded-[28px] border border-white/10 bg-white/[0.04] p-5 shadow-glow backdrop-blur-xl sm:rounded-[32px] sm:p-6 lg:w-[370px]">
          ...
          <div className="relative space-y-6">
            <div>
              ...
              <h1 className="text-4xl font-black uppercase tracking-[0.18em] text-white sm:text-5xl">
                Jogo
              </h1>
              <p className="mt-3 max-w-sm text-sm leading-6 text-slate-300">
                Colete os pontos, aumente a cobra e evite colisões com o próprio
                corpo.
              </p>
            </div>
        ...
        <section className="relative rounded-[28px] border border-white/10 bg-slate-950/60 p-3 shadow-glow backdrop-blur-xl sm:rounded-[36px] sm:p-4">
          ...
          <div className="relative space-y-4">
            ...
            <div className="grid grid-cols-3 grid-rows-2 gap-3 sm:hidden">
              {touchControls.map(
                ({ label, ariaLabel, direction: nextDirection, className }) => (
                  <button
                    key={label}
                    type="button"
